...
function formatAddress(address: SiteSettingsGlobal['address']): string {
  if (!address) return ''
  const { street, suburb, state, postcode } = address
  return [street, suburb && state ? `${suburb} ${state} ${postcode}` : suburb].filter(Boolean).join(', ')
}
...
export async function ContactCTA() {
  ...
  const addressStr = formatAddress(settings.address) || formatAddress(FALLBACK_SETTINGS.address)
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <FadeUp delay={0.22}>
            <div className="flex flex-wrap items-center justify-center gap-4">
              ...
              <a href={phoneHref} className="btn-outline flex items-center gap-2">
